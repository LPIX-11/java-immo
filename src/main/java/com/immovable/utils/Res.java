package main.java.com.immovable.utils;
...
public class Res {
    ...
    public Picture writeToFile(InputStream uploadedInputStream, ContentDisposition contentDisposition) throws IOException {
//        String uploadLocation = System.getProperty("user.home") + "/img/";
    ...
    public static String generateNumBien() {
        try {
            int a = (int) HibernateConfiguration.getSession().createQuery("SELECT MAX(b.id) FROM Property b").getSingleResult();
    ...
    public static String generateMatriculeClient() {
        try {
            int a = (int) HibernateConfiguration.getSession().createQuery("SELECT MAX(c.id) FROM Customer c").getSingleResult();
            NumberFormat nf = new DecimalFormat("0000");
            int b = a + 1;
            String matricule = nf.format(b);
